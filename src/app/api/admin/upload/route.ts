...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!isImage && !isVideo) {
      return NextResponse.json(
        { ok: false, error: `Unsupported file type ".${ext}". Allowed: jpg, png, webp, gif, mp4, webm, mov.` },
        { status: 400 }
      );
    }
    ...
    if (isImage && file.size > MAX_IMAGE) {
      return NextResponse.json(
        { ok: false, error: `Image is ${(file.size / 1024 / 1024).toFixed(1)}MB — max is 10MB.` },
        { status: 413 }
      );
    }
    ...
    if (isVideo && file.size > MAX_VIDEO) {
      return NextResponse.json(
        { ok: false, error: `Video is ${(file.size / 1024 / 1024).toFixed(1)}MB — max is 100MB.` },
        { status: 413 }
      );
    }
    ...
    await logActivity("create", "media", media.id, `Uploaded ${file.name} (${(file.size / 1024).toFixed(0)}KB)`);
