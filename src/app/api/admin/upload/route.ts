...
// Allow large uploads (videos up to 100MB). Default Next.js body parser limit
// is ~4MB which would reject most videos. Setting this high so the route
// handler can enforce its own limits.
...
export async function POST(req: NextRequest) {
  ...
  try {
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            "Could not read upload. The file may be too large for the server.",
        },
        { status: 400 }
      );
    }
    ...
    // For images, get dimensions with sharp
