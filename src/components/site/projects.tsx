...
export function Projects({ projects }: { projects: ProjectItem[] }) {
  ...
  // ─── GSAP refs for header + filter tabs ─────────────────────
  const headerRef = useRef<HTMLDivElement>(null);
  const ctaRef = useRef<HTMLDivElement>(null);
  const tabsRef = useRef<HTMLDivElement>(null);

  // Header: fade + slide up. CTA: delayed reveal. Tabs: stagger from top.
  useGsapReveal(headerRef, { delay: 0, y: 36, duration: 0.9 });
  useGsapReveal(ctaRef, { delay: 0.2, y: 24, duration: 0.7 });
  useGsapStagger(tabsRef, "[data-tab]", {
    stagger: 0.08,
    y: 16,
    from: "top",
  });
  ...
  return (
    <section id="projects" className="bg-[#F3F4F6] py-24 md:py-32">
      <div className="container-drill">
        ...
        {/* ──────────────  Filter tabs — GSAP stagger from top  ────────────── */}
        <div ref={tabsRef} className="mt-12 will-change-transform">
          <div
            ...
            {filterTabs.map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`relative h-10 px-4 text-[12px] font-semibold uppercase tracking-wide transition-colors will-change-transform md:h-11 md:px-5 md:text-[13px] ${
                    isActive
        ...
        {/* ──────────────  Masonry grid — GSAP stagger + ken-burns  ────────────── */}
        <ProjectsGrid
          key={activeFilter}
...
/* ---------- Masonry grid — re-mounts on filter change for fresh GSAP stagger ---------- */
function ProjectsGrid({
  ...
  // Stagger all project cards on mount (re-fires per filter change thanks to key).
  useGsapStagger(gridRef, "[data-project]", { stagger: 0.1, y: 32 });

  return (
    <div
      ref={gridRef}
      className="mt-10 columns-1 gap-4 sm:columns-2 lg:columns-3 md:gap-6 [column-fill:_balance]"
    >
...
/* ---------- Individual project card — per-card GSAP ken-burns + badge reveal ---------- */
function ProjectCard({
  ...
  // Card image: ken-burns scale-down from 1.2 → 1 on reveal.
  useGsapScale(imageRef, { delay: 0.2 + (idx % 6) * 0.05, from: 1.2 });
  // Category badge: fade + slide up reveal alongside the card.
  useGsapReveal(badgeRef, {
    delay: 0.4 + (idx % 6) * 0.05,
    y: 12,
    duration: 0.55,
  });
  ...
  return (
    <motion.button
      ...
      className="group relative mb-4 block w-full break-inside-avoid overflow-hidden bg-[#121117] text-left will-change-transform md:mb-6"
    ...
    >
      <div className={`relative overflow-hidden ${aspect}`}>
        ...
        {/* Category badge top-left (GSAP slide-in from left) */}
        <span
          ref={badgeRef}
          className="absolute left-4 top-4 bg-[#121117] px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-white will-change-transform md:text-[11px]"
        >
          {project.category}
        </span>
...
/* ---------- Lightbox modal — GSAP clip-in on image + Framer Motion backdrop/card ---------- */
function LightboxModal({
  ...
  // Image: GSAP clip-path wipe on mount.
  const imageWrapRef = useRef<HTMLDivElement>(null);
  useGsapClipReveal(imageWrapRef, { delay: 0.15, direction: "right" });
  ...
  return (
    ...
    >
      ...
      {/* Card — Framer Motion scale + opacity (GSAP handles the image clip-in) */}
      <motion.div
      ...
      >
        <div
          ref={imageWrapRef}
          className="relative aspect-[16/10] w-full overflow-hidden bg-[#121117] will-change-transform"
        >
          <img
            src={project.image}
            alt={`${project.title} — ${project.category} in ${project.location}`}
