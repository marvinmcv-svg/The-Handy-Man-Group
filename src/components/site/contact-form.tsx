...
import { Phone, Mail, MapPin, Clock, Check, Loader2, AlertCircle, Instagram } from "lucide-react";
            ...
            {status === "success" ? (
              ...
              >
                <motion.div
                  ...
                  className="flex h-16 w-16 items-center justify-center bg-[#D2151E] text-white"
                >
              ...
              <form onSubmit={handleSubmit} className="space-y-5" noValidate>
                <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
                  ...
                  <Field label="Phone" htmlFor="phone" required>
                    <input id="phone" name="phone" type="tel" required autoComplete="tel" placeholder="04xx xxx xxx" className="drill-input" />
                  </Field>
                </div>
                <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
                  <Field label="Email" htmlFor="email" required>
                    <input id="email" name="email" type="email" required autoComplete="email" placeholder="jane@example.com" className="drill-input" />
                  </Field>
                  <Field label="Suburb" htmlFor="suburb">
                    <input id="suburb" name="suburb" type="text" autoComplete="address-level2" placeholder="e.g. Paddington, QLD" className="drill-input" />
                  </Field>
                </div>
                <Field label="What do you need?" htmlFor="service" required>
                  <select id="service" name="service" required defaultValue="" className="drill-input appearance-none bg-white">
                ...
                <Field label="Project details" htmlFor="message" required hint="Tell us what you're trying to achieve, rough timing, and anything else useful.">
                  <textarea id="message" name="message" required rows={5} minLength={10} placeholder="e.g. We'd like to refresh our kitchen — new cabinetry, benchtop and a coat of paint. Looking to start in about 4 weeks." className="drill-input resize-y" />
                </Field>
                ...
                {status === "error" && (
                  <motion.div
                    ...
                    className="flex items-start gap-3 border border-[#D2151E] bg-[#FFDEDE] p-4"
                  >
                    <AlertCircle className="mt-0.5 h-5 w-5 shrink-0 text-[#D2151E]" />
                    <p className="text-[14px] font-normal text-[#121117]">{message}</p>
                  </motion.div>
                )}
...
function Field({
  ...
  return (
    <div>
      <label htmlFor={htmlFor} className="mb-2 block text-[13px] font-semibold text-[#121117]">
        {label}
        {required && <span className="ml-1 text-[#D2151E]">*</span>}
      </label>
      {children}
      {hint && <p className="mt-1.5 text-[12px] font-normal text-[#999999]">{hint}</p>}
    </div>
  );
}
